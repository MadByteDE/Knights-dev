...
function Object:init(x, y, t)
  ...
  self.transform  = self.transform or {r=0, sx=1, sy=1, ox=0, oy=0}
  self.dimension  = self.dimension or {w=8, h=8}
  self.position   = {x=x+self.transform.ox or 0, y=y+self.transform.oy or 0}
...
function Object:getRect(x, y, w, h)
  ...
  local w, h = w or self.dimension.w, h or self.dimension.h
  return x, y, w, h
end
...
function Object:drawRectangle(mode)
  ...
  love.graphics.rectangle(mode or "fill", self.position.x, self.position.y, self.dimension.w, self.dimension.h)
...
function Object:draw()
  if self.visible and self.sprite then
    local r = self.transform.r
    local x, y = self:getRect()
    local sx, sy = self.transform.sx, self.transform.sy
    local ox, oy = self.transform.ox, self.transform.oy
    love.graphics.setColor(self.rgba)
